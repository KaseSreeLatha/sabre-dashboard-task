...
const getStatus = (isDataUploaded: boolean | null | undefined) => {
    if (isDataUploaded === true) return 'success';
    if (isDataUploaded === false) return 'error';
    return 'warning';
};
...
const SeasonGrid: React.FC<SeasonGridProps> = ({ data }) => {
    return (
        <div
            className={`season-column ${data.name.toLowerCase().includes('winter') ? 'winter-padding' : 'summer-padding'
                }`}
        >
